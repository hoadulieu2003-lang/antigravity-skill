/**
 * ⚡ LUMINOUS PALETTE ENGINE (Universal UMD / Standalone Global Bundle)
 * Antigravity 2.0 Autonomous Light Theme Color Engine
 */
(function (global, factory) {
  if (typeof exports === 'object' && typeof module !== 'undefined') {
    ...
    global.LuminousPaletteEngine = exp.LuminousPaletteEngine;
    global.generateLuminousPalette = exp.generateLuminousPalette;
    global.getCssVariables = exp.getCssVariables;
  }
...
/**
 * Parse hex color string to normalized RGB [0..1]
 * Supports #RGB, #RGBA, #RRGGBB, #RRGGBBAA and raw hex without hash.
 */
 ...
 * Prevents electric volt / lemon yellow (h in [85, 125]) from decomposing into
 * murky drab olive when lightness is reduced. Smoothly bends hue towards
 * rich warm amber / gold (65° - 72°) as Lightness decreases from 0.85 down to 0.45.
 */
function bendPerceptualHue(L, h, options = {}) {
  ...
  // Volt / Lemon / Lime-Yellow zone [85, 125]
  if (normH >= 85 && normH <= 125) {
    ...
    if (L <= lowThreshold) {
      return targetAmberHue;
    }

    // Smooth cubic hermite curve (smoothstep)
    const t = (highThreshold - L) / (highThreshold - lowThreshold);
    const smoothT = t * t * (3 - 2 * t);
    return normH + (targetAmberHue - normH) * smoothT;
  }
 ...
 * Searches for the maximum Lightness L in [0.02, 0.85] with precision < 10^-5
 * that guarantees contrast >= targetRatio on all target background luminances.
 */
 ...
 * Given any background hex, finds high-contrast accessible text color.
 */
function resolveAccessibleTextColor(bgHex, options = {}) {
  const bgY = hexToRelativeLuminance(bgHex);
  const targetRatio = options.targetRatio ?? 4.5;
  const seedHex = options.seedHex || '#0F172A';
...
function getCssVariables(palette, options = {}) {
  const map = palette.cssVariables || getCssVariablesMap(palette, options);
  if (options.asObject) {
    return map;
  }
