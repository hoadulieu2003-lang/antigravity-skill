...
(async () => {
  try {
    ...
    // Check if there is a button for DESIGN_TRAINING_007_REVIEW_001.md
    const reviewBtn = await chatPage.evaluate(() => {
      const btns = Array.from(document.querySelectorAll('button'));
      const found = btns.find(b => 
        (b.getAttribute('aria-label') && b.getAttribute('aria-label').includes('DESIGN_TRAINING_007_REVIEW_001')) ||
        (b.innerText && b.innerText.includes('DESIGN_TRAINING_007_REVIEW_001'))
      );
      if (found) {
        found.click();
        return true;
    ...
    if (reviewBtn) {
      ...
      if (canvasContent && canvasContent.length > 100) {
        ...
        fs.writeFileSync('C:/Users/game/.gemini/exercises/design_training_007/DESIGN_TRAINING_007_REVIEW_001.md', canvasContent, 'utf8');
        console.log('Saved Canvas Content to DESIGN_TRAINING_007_REVIEW_001.md');
      } else {
        console.log('No canvas content found, saving message text instead.');
        fs.writeFileSync('C:/Users/game/.gemini/exercises/design_training_007/DESIGN_TRAINING_007_REVIEW_001.md', lastMsgInfo.text, 'utf8');
      }
    } else {
      fs.writeFileSync('C:/Users/game/.gemini/exercises/design_training_007/DESIGN_TRAINING_007_REVIEW_001.md', lastMsgInfo.text, 'utf8');
    }
