...
async function run() {
  ...
  // 1. Check file attachment
  const checkAttachment = await sendCdp(ws, 'Runtime.evaluate', {
    expression: `document.body.innerText.includes('design_training_007_submission_r01.zip')`,
    returnByValue: true
  });
  console.log('[STATE] File attached in DOM:', checkAttachment.result?.value);

  if (!checkAttachment.result?.value) {
    console.log('[ATTACH] Attaching file...');
    ...
    console.log('[ATTACH] FileInput nodeId:', fileInput.nodeId);
  ...
  // 2. Check textarea content
  const checkText = await sendCdp(ws, 'Runtime.evaluate', {
    expression: `(() => {
  ...
  console.log('[STATE] Textarea length:', checkText.result?.value);

  if (!checkText.result?.value || checkText.result?.value < 100) {
  ...
  // 3. Record current assistant message count
  ...
  console.log(`[STATE] Assistant message count before dispatch: ${initialAssistantCount}`);

  // 4. Click Send using mousedown + mouseup + click
  console.log('[SEND] Triggering send button...');
  ...
  console.log('[SEND] Button click result:', sendRes.result?.value);

  await new Promise(r => setTimeout(r, 1500));

  // Check if sent, if not press Enter
  const checkSent = await sendCdp(ws, 'Runtime.evaluate', {
    expression: `(() => {
      const isGen = document.querySelector('button[aria-label*="Stop generating"], button[data-testid="stop-button"]') !== null;
      const el = document.getElementById('prompt-textarea');
      return { isGenerating: isGen, textLen: el ? el.innerText.length : 0 };
    })()`,
    returnByValue: true
  });
  console.log('[CHECK] After click:', checkSent.result?.value);

  if (checkSent.result?.value?.textLen > 0 && !checkSent.result?.value?.isGenerating) {
    console.log('[SEND] Text still present, dispatching Enter key via Input.dispatchKeyEvent...');
    await sendCdp(ws, 'Runtime.evaluate', { expression: `document.getElementById('prompt-textarea')?.focus()` });
  ...
  console.log('[POLL] Polling for ChatGPT response...');
  let finished = false;
  for (let m = 0; m < 120; m++) {
