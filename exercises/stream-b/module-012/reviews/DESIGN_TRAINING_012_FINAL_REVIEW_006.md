...
Đưa snapshot R04 thật vào đúng source_snapshot/ và xác minh hash e76ab08f....

Thay assertion tên-path bằng metadata bất biến trong package, ví dụ stream_id: B, danh sách path cho phép và kiểm tra không có stream-a/.

Runner phải PASS khi giải nén vào một thư mục ngẫu nhiên bất kỳ.

Không đọc source snapshot từ workspace tác giả bên ngoài ZIP.
...
Tạo canonical fixture/allowlist độc lập, machine-readable.

Trích xuất từng row T01–T08 theo cấu trúc DOM và deep-compare giá trị thực, không dùng six independent includes() trên toàn trang.

Mọi operational fact trong HTML/SVG/YAML/alt/desc phải thuộc canonical fixture hoặc một fictional fixture được gắn nhãn và đăng ký rõ.

Thêm negative fixture đưa một fact không hợp lệ vào bản sao tạm; validator bắt buộc FAIL.

Không được sửa Direction A/B đã khóa; nếu cần làm sạch asset dùng cho candidate, tạo candidate-specific asset/manifest entry thay vì làm thay đổi prototype nguồn.
...
Áp dụng mọi compliance/evidence/responsive patch chung lên cả pre_critique.html và index.html.

Giữ final diff đúng tối đa hai thay đổi thiết kế liên quan trực tiếp đến một hypothesis.

Với hypothesis hiện tại, phạm vi an toàn là:

thay đổi treatment viền alert;

thay đổi treatment CTA chính.

Loại bỏ hoặc đồng bộ các thay đổi title, badge, padding, shadow ngoài hypothesis.

Runner phải xuất normalized diff summary và assertion về critique scope.
...
source snapshot thật trong ZIP;

portable executable runner và dependency/launch instructions;

clean-run console log từ một thư mục giải nén mới;

VERIFICATION.json sinh từ clean run, không chỉnh tay;

canonical allowlist/schema + positive/negative fixture evidence;

runtime measurements cho 1440/768/390;

request-failure evidence;

normalized critique diff evidence;

10 screenshot mới và manifest hash/dimension/DPR;

report đã đồng bộ với actual results;

package filename, bytes, file count và SHA-256.
