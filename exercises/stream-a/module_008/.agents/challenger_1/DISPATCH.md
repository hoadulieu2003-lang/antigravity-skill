...
Your assigned working directory is: C:\Users\game\.gemini\exercises\stream-a\module_008\.agents\challenger_1
...
Task Objective (Challenger 1 — Adversarial Contrast & Token Stress Testing):
1. Adversarial Contrast Math Verification:
   - Write a standalone test script in your working directory to recalculate W3C sRGB relative luminance and contrast ratios across all 15 color pairs from `COLOR_CONTRACT.yaml` and live DOM elements of `index.html`, `option_a.html`, and `option_b.html`.
   - Test against floating point precision edge cases and rounding boundaries.
2. Adversarial Token Provenance Audit:
   - Stress test all CSS rules in the HTML files: check for any sneaky primitive references in inline styles, SVG fills/strokes, or nested selectors.
3. Run verification:
