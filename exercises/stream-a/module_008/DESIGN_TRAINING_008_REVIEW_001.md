...
Search panel có khoảng trắng dọc rất lớn; icon tìm kiếm nằm giữa vùng trống, tách khỏi input.

Nguyên nhân source: .search-box giữ flex: 1 1 280px khi parent chuyển sang column; media rule chỉ thêm width: 100% nên flex-basis trở thành chiều cao bất thường.

Ledger vẫn là bảng nhiều cột nằm trong .table-responsive-wrapper { overflow-x: auto; }; ảnh chỉ hiển thị một phần cột và status badge bị cắt ở cạnh phải.

body { overflow-x: hidden; } có thể che overflow toàn trang thay vì chứng minh layout không tạo overflow.
