...
Tạo semantic token còn thiếu cho table-row-hover, filter surface/text và shadow color/value hoặc xác định rõ shadow là non-color token được quản trị ở tầng thích hợp.

Mọi --dispatch-* phải tham chiếu --color-* hoặc semantic token hợp lệ; không gọi --primitive-*, không chứa hex/rgb/rgba màu trực tiếp.

Harness phải parse riêng các khai báo --dispatch-* trong :root của cả Candidate và hai direction.

C01 phải FAIL closed nếu contract thiếu, tier thiếu, component token unresolved, primitive leakage tồn tại hoặc các cờ taxonomy/brand segregation chưa được tính từ source. Không gán true cố định.
...
Tạo rendered_role_inventory từ DOM/CSS Candidate, với mỗi role trỏ tới một pair_id đã đo.

Hover: dùng page.hover(exactSelector) rồi đọc getComputedStyle.

Active: dùng pointer down giữ trạng thái hoặc một cơ chế runtime thực, đọc computed style trong lúc :active, sau đó pointer up.

Placeholder: dùng getComputedStyle(input, '::placeholder') nếu Chromium hỗ trợ; nếu không, ghi UNSUPPORTED và không tính như computed measurement.

Focus: tiếp tục dùng kết quả C05 native Tab; không lặp lại bằng hex cứng trong C03.

Selector không tồn tại, pseudo-state không kích hoạt, màu transparent không resolve hoặc phép đo lỗi phải làm Gate FAIL closed; cấm fallback sang màu mặc định.

Bổ sung emergency action và các state thực tế còn thiếu; phân loại rõ text, large text, meaningful non-text và decorative.
...
Đưa tiêu đề bảng ra ngoài table thành heading/label block, hoặc đặt caption mobile/tablet thành block/full inline-size theo cách semantic hợp lệ.

Caption phải đọc theo dòng tự nhiên tại 768px và 390px, không tạo vùng trắng lớn.

Giữ nguyên stacked-card structure, đủ sáu trường và zero local/global horizontal overflow.

Harness không được coi cells.length >= 6 là “readable”. Bổ sung rect/visibility checks: mỗi value có width/height dương, nằm trong row/card, không bị clip, không tạo local overflow; caption width phải đạt ngưỡng hợp lý so với ledger container.

Chụp lại tablet full page, mobile full page và mobile first viewport.
...
Dùng một implementation có provenance rõ, ghi chính xác tên thuật toán/version, không gian màu, transformation pipeline và trích dẫn tới nguồn chứa/derive đúng hệ số; hoặc

Giữ ma trận hiện tại nhưng đổi tên trung thực thành MODULE_APPROXIMATION_MATRIX, bỏ attribution Brettel/Machado và nói rõ đây chỉ là heuristic visualization không được xác thực khoa học.
...
Giữ một action button node xuyên suốt IDLE → VALIDATING → SAVING → FAILURE → retry → CONFIRMED; chỉ đổi label/content/state attributes.

Không thay node bằng innerHTML của container.

Không gọi this.focus() vô điều kiện sau timer. Nếu người dùng đã Tab đi nơi khác, success không được cướp focus.

Dùng native keyboard/pointer trong harness; ghi node identity, active element trong first saving, failure, retry saving, success và một no-steal scenario.

Đồng bộ COLOR_CONTRACT.yaml với implementation thực tế.
...
Đưa nguyên bản byte-identical DESIGN_TRAINING_008_REVIEW_001.md và DESIGN_TRAINING_008_REVIEW_002.md vào gói cuối.

Không reflow, copy lại từ chat hoặc sửa formatting tài liệu Controller.

Report chỉ ghi SELF_CHECK_PASS hoặc SUBMITTED_FOR_CONTROLLER_REVIEW; bỏ mọi câu tự chứng nhận đã đóng hoàn toàn/triệt để.

Đồng bộ source hashes, ảnh, JSON, report và contract sau mọi sửa đổi.
