...
"""
════════════════════════════════════════════════════════════════════════════
TURBO UGC MASTER NEWS VIDEO COMPOSITOR (AUTHENTIC THREADS FOOTAGE)
Hardware-accelerated NumPy slice blending.
Renders 1080x1920 Full HD at 80-100 FPS directly into FFmpeg pipe!
════════════════════════════════════════════════════════════════════════════
"""
...
def build_segment_overlay(seg):
    ...
    draw.text((380, 75), "HIỆN TRƯỜNG THỰC TẾ", fill=(255, 255, 255, 240), font=font_clock)
    ...
    # Tách kênh màu BGR và Alpha float
...
def compile_master_ugc_video():
    print("================================================================")
    print("🚀 KHỞI ĐỘNG TURBO RENDER VIDEO TIN TỨC THREADS (80-100 FPS)...")
    print("================================================================")

    # 1. Pre-build overlay layers cho tất cả 5 phân cảnh
    print("[*] Đang biên dịch trước Card đồ họa chuẩn VTV cho 5 phân cảnh...")
    cached_overlays = []
    for seg in SEGMENTS:
        bgr, alpha, inv_alpha = build_segment_overlay(seg)
    ...
    for seg_idx, seg in enumerate(SEGMENTS):
        ...
        vpath = seg["video"]
        overlay = cached_overlays[seg_idx]
        ov_bgr = overlay["bgr"]
        ov_alpha = overlay["alpha"]
        ov_inv_alpha = overlay["inv_alpha"]

        print(f"[*] Phân cảnh #{seg_idx+1}/{len(SEGMENTS)}: {seg['badge']} ({seg_frames} frames)...")
        ...
        for f_i in range(seg_frames):
            ...
            # NumPy hardware-speed blending: Chỉ blend vùng đỉnh (0:280) và vùng đáy (H-550:H)
            final_frame = frame_bgr.copy()
            # Top blend
            final_frame[:280] = (frame_bgr[:280].astype(np.float32) * ov_inv_alpha[:280] + ov_bgr[:280] * ov_alpha[:280]).astype(np.uint8)
            # Bottom blend
            final_frame[H-550:] = (frame_bgr[H-550:].astype(np.float32) * ov_inv_alpha[H-550:] + ov_bgr[H-550:] * ov_alpha[H-550:]).astype(np.uint8)
