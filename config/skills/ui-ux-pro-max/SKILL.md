---
name: ui-ux-pro-max
description: "UI/UX intelligence database for UX rules, accessibility standards (WCAG AA), typography pairings, color palette reasoning, and interaction guidelines. Activate when searching for design tokens, font pairings, UX best practices, or auditing component usability. DO NOT use for high-level art direction ($design) or brand-mimicking templates (open-design)."
---
...
```bash
python "C:\Users\game\.gemini\config\skills\ui-ux-pro-max\scripts\search.py" "<query>" --domain <domain>
# Hoặc tạo trọn gói Design System:
python "C:\Users\game\.gemini\config\skills\ui-ux-pro-max\scripts\search.py" "<product_type> <keywords>" --design-system
```
