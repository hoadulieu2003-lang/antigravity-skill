/**
 * ⚡ WEBGL MICRO-ENGINE — Antigravity 2.0 Ultra-Lightweight 3D WebGL / Canvas Micro-Engine
 * =======================================================================================
 * Ultra-lightweight (<4 KB minified/gzip, 0 KB external assets, zero dependencies).
 * Pure JavaScript 3D procedural shader & particle engine designed for high-performance web UIs.
 * 100% SSR-safe: Seamlessly runs in standalone HTML, Vite, React, Vue, and Next.js.
 *
 * Core Modules:
 *  1. Iridescent Liquid Light Waves (initLiquidWaves):
 *     - Procedural domain-warped harmonic waves with pearlescent thin-film interference.
 *     - Luminous light theme compliant (background luminance >= 0.90).
 *     - ZERO Reinhard tone mapping (no dull gray contrast crushing).
 *
 *  2. Interactive Particle Mesh (initParticleMesh):
 *     - Dynamic micro-particle network reacting to mouse proximity with spring physics.
 *     - 4-Tier Quantized Alpha Stroke Buckets (strictly <= 4 stroke() calls per frame, zero popping).
 *     - Non-destructive smooth position rescaling across DPR / window resizes.
 *
 *  3. Titanium Perspective Grid (initTitaniumGrid):
 *     - 3D perspective infinite ground plane with horizon projection.
 *     - Blinn-Phong specular highlight reacting dynamically to mouse cursor.
 *     - fwidth standard derivative anti-aliasing (strictly GLSL compliant edge0 < edge1).
 *
 * Performance & Accessibility:
 *  - FrameBudgetWatchdog: 60 FPS runtime monitor with automatic DPR downscaling & NaN safety.
 *  - IntersectionObserver: Managed auto-pause/resume with strict immunity against overriding
 *    user accessibility preferences (prefers-reduced-motion) and manual developer pause().
 *  - Complete lifecycle management with idempotent destroy() cleanup.
 *
 * License: Apache-2.0
 */
...
// =============================================================================
// 1. FRAME BUDGET WATCHDOG (60 FPS Performance Protection & Dynamic DPR)
// =============================================================================

export class FrameBudgetWatchdog {
  constructor(options = {}) {
    this.targetFps = options.targetFps || 60;
    this.targetFrameTime = 1000 / this.targetFps; // 16.666 ms
    this.budgetCeiling = options.budgetCeiling || 20.0; // 50 FPS threshold
  ...
  recordFrame(now) {
    ...
    // Ignore tab switches or huge pauses
    if (delta > 250) {
      return;
    }
  ...
  stepDown() {
    ...
    if (currIdx !== -1 && currIdx < this.dprSteps.length - 1) {
      ...
      if (nextDpr !== this.currentDpr) {
        ...
        this.cooldownFrames = 120; // 2 seconds stabilization
        this.slowFrameStreak = 0;
        this.history = [];
...
// =============================================================================
// 2. PROCEDURAL SHADER SOURCES (GLSL Shaders with Luminous Light Invariants)
// =============================================================================
...
void main() {
  ...
  // Procedural Shader 1: Iridescent Liquid Light Waves
  // - Background luminance >= 0.90 (default Warm Paper #FAF9F6: vec3(0.980, 0.976, 0.965))
  // - ZERO Reinhard tone mapping (no col / (col + 1.0))
  // - Additive subtle spectral pearlescence
...
void main() {
  ...
  // Procedural Shader 3: Titanium Perspective Grid
  // - 3D depth ground plane
  // - Blinn-Phong specular highlight reacting to cursor
  // - fwidth standard derivative anti-aliasing (strictly GLSL compliant 1.0 - smoothstep(min, max, x))
...
// =============================================================================
// 3. WEBGL CONTEXT & SHADER COMPILATION HELPER
// =============================================================================
...
// =============================================================================
// 4. SHARED WEBGL QUAD SHADER ENGINE (Iridescent Waves & Titanium Grid)
// =============================================================================
...
export function initLiquidWaves(target, options = {}) {
  return createQuadShaderEngine(target, SHADERS.LIQUID_WAVES_FS, {
    baseColor: [0.980, 0.976, 0.965], // Warm Paper #FAF9F6 (Luminance ~0.976 >= 0.90)
    speed: 0.22,
    intensity: 1.0,
  }, options);
}

export function initTitaniumGrid(target, options = {}) {
  return createQuadShaderEngine(target, SHADERS.TITANIUM_GRID_FS, {
    baseColor: [0.976, 0.980, 0.984], // Alabaster / Ivory light background
    speed: 0.45,
    intensity: 1.0,
...
// =============================================================================
// 5. MODULE 2: INTERACTIVE PARTICLE MESH (4-Tier Quantized Alpha Buckets)
// =============================================================================

export function initParticleMesh(target, options = {}) {
  const canvas = resolveCanvas(target);
  if (!canvas) {
    return {
      destroy: () => {},
  ...
  const baseColor = options.baseColor || [30, 41, 59]; // Slate 800 for high-contrast on light background
  const speed = options.speed || 0.45;
  const quantizedAlphas = options.quantizedAlphas || [0.08, 0.18, 0.32, 0.50]; // Exactly 4 tiers
  ...
  const particles = [];
  const buckets = [[], [], [], []]; // 4 quantized alpha stroke buckets
  ...
  function update() {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      // Proximity spring / repulsion reaction
      if (mActive) {
        const dx = p.x - mx;
        const dy = p.y - my;
  ...
  function draw() {
    ...
    // Pairwise distance test & 4-tier quantization
    for (let i = 0; i < count; i++) {
      ...
      for (let j = i + 1; j < count; j++) {
        ...
        if (distSq < maxDistSq) {
          ...
          const ratio = dist / maxDist; // 0 (closest) to 1 (farthest)
    ...
    // 4-TIER QUANTIZED ALPHA STROKE BUCKETS: Exactly <= 4 stroke() calls per frame
    ...
    for (let t = 0; t < 4; t++) {
      ...
      ctx.stroke(); // EXACTLY 1 stroke() call per tier
      strokes++;
    }
...
// =============================================================================
// 6. MASTER WEBGL MICRO-ENGINE WRAPPER & AUTO-ATTACHMENT
// =============================================================================

export const WebGLMicroEngine = {
  version: '2.0.0',
  init(target, options = {}) {
...
if (isBrowser()) {
  window.WebGLMicroEngine = WebGLMicroEngine;
  window.initLiquidWaves = initLiquidWaves;
  window.initParticleMesh = initParticleMesh;
  window.initTitaniumGrid = initTitaniumGrid;
  window.FrameBudgetWatchdog = FrameBudgetWatchdog;

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      if (document.querySelector('[data-micro-engine]')) {
        WebGLMicroEngine.initAll();
      }
    });
  } else if (document.querySelector('[data-micro-engine]')) {
    WebGLMicroEngine.initAll();
  }
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = WebGLMicroEngine;
  module.exports.WebGLMicroEngine = WebGLMicroEngine;
  module.exports.initLiquidWaves = initLiquidWaves;
  module.exports.initParticleMesh = initParticleMesh;
  module.exports.initTitaniumGrid = initTitaniumGrid;
  module.exports.FrameBudgetWatchdog = FrameBudgetWatchdog;
  module.exports.default = WebGLMicroEngine;
}
