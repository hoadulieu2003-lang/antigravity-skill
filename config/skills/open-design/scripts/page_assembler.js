...
export function sanitizeUrl(rawUrl, fallback = '#') {
  ...
  // Decode URI component to uncover hidden scheme payloads (e.g. javascript:)
  let decoded = trimmed;
  try {
  ...
  if (/[\0-\x1F\x7F<>"'`\\]/.test(decoded)) return fallback;

  // Protocol-relative URL check: '//example.com' (MUST be before startsWith('/'))
  ...
  // Check explicit protocol (both on raw and decoded)
  const colonIndex = decoded.indexOf(':');
  if (colonIndex !== -1) {
    const protocol = decoded.substring(0, colonIndex).trim().toLowerCase();
 ...
 * @returns {T}
 */
export function safeDeepMerge(target, ...sources) {
  if (target === null || typeof target !== 'object' || Object.isFrozen(target)) {
    target = /** @type {any} */ (Array.isArray(target) ? [...(target || [])] : { ...(target || {}) });
  }
  ...
  for (const source of sources) {
    ...
    for (const key of keys) {
      ...
      if (Array.isArray(val)) {
        ...
        if (!existing || typeof existing !== 'object' || Array.isArray(existing) || Object.isFrozen(existing)) {
          /** @type {any} */ (target)[key] = existing && typeof existing === 'object' && !Array.isArray(existing)
            ? { ...existing }
            : {};
        }
...
const RE_NAMED_COLOR = /^(transparent|currentcolor|inherit|initial|unset|none|white|black|slate|zinc|gray|neutral|stone|red|orange|amber|yellow|lime|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose)$/i;
const RE_DIMENSION = /^-?\d+(\.\d+)?(px|rem|em|vh|vw|vmin|vmax|%|deg|turn|rad|ms|s|fr)$/i;
const RE_ZERO = /^0$/;
const RE_KEYWORDS = /^(auto|inherit|initial|unset|normal|dense)$/i;
...
const RE_CALC = /^(calc|min|max|clamp)\(\s*[-+*/0-9.,%\s(px|rem|em|vh|vw|vmin|vmax|deg|fr|s|ms)]+\)$/i;

/**
 * Validates a safe CSS gradient function
 * @param {string} str
 * @returns {boolean}
 */
function isValidGradient(str) {
  if (!/^(linear-gradient|radial-gradient|conic-gradient)\(/i.test(str)) return false;
  if (!str.endsWith(')')) return false;
  if (DANGEROUS_CSS_PATTERNS.test(str)) return false;
...
/**
 * Validates a font family stack.
 * Ensures font stack is composed of valid font tokens and contains at least
 * one recognized generic family or recognized system font family.
 * Prevents arbitrary alphanumeric words (e.g. 'notAColor', 'foo bar baz') from passing as valid CSS.
 *
 * @param {string} str
 * @returns {boolean}
 */
function isValidFontStack(str) {
  if (!/^[a-zA-Z0-9\-_,\s]+$/.test(str)) return false;
  const lower = str.toLowerCase();
  const hasRecognizedFont = /(sans-serif|serif|monospace|system-ui|-apple-system|blinkmacsystemfont|segoe ui|roboto|helvetica|arial|inter|sfmono-regular|menlo|monaco|consolas)/i.test(lower);
  return hasRecognizedFont;
}
 ...
 * @returns {string}
 */
export function sanitizeCssValue(val, fallback = '') {
  ...
  // 4. Safe calc(), min(), max(), clamp() expressions
  if (RE_CALC.test(trimmed)) {
    return trimmed;
  }
  ...
  // 6. Box shadows (support multi-layer comma separated shadows)
  ...
  // 7. General CSS keywords
  if (RE_KEYWORDS.test(trimmed)) {
    return trimmed;
  }

  // 8. Recognized Font stack
  if (isValidFontStack(trimmed)) {
    return trimmed;
  }
...
export const DEFAULT_LUMINOUS_THEME = Object.freeze({
  canvas: '#FAF9F6',
  surface: '#FFFFFF',
  elevated: '#FFFFFF',
...
export const DEFAULT_PAGE_SCHEMA = Object.freeze({
 ...
 * @returns {any}
 */
export function createVirtualElement(tagName) {
  ...
  const element = {
    ...
    querySelectorAll: (selector) => {
      const selectorGroups = selector.split(',').map((s) => s.trim()).filter(Boolean);
        ...
        for (const part of parts) {
          ...
          const matchCompound = (el, token) => {
            ...
            if (tagMatch) {
              ...
              rem = rem.slice(tagMatch[0].length);
            }

            while (rem.length > 0) {
              if (rem.startsWith('#')) {
                const idMatch = rem.match(/^#([a-zA-Z0-9\-_]+)/);
                if (!idMatch) return false;
                if (!el.getAttribute || el.getAttribute('id') !== idMatch[1]) return false;
                rem = rem.slice(idMatch[0].length);
              } else if (rem.startsWith('.')) {
                const clsMatch = rem.match(/^\.([a-zA-Z0-9\-_]+)/);
                if (!clsMatch) return false;
                if (!el.classList || !el.classList.contains(clsMatch[1])) return false;
                rem = rem.slice(clsMatch[0].length);
              } else if (rem.startsWith('[')) {
                const attrMatch = rem.match(/^\[([a-zA-Z0-9\-_]+)(?:([~|^$*]?=)(["']?)(.*?)\3)?\]/);
                ...
                if (operator === '=' && actualVal !== expectedVal) return false;
                rem = rem.slice(attrMatch[0].length);
              } else {
                return false;
              }
            }
 ...
 */
export function normalizeSchema(userSchema) {
 ...
 * @param {any} theme
 */
export function applyThemeVariables(targetElement, theme) {
 ...
 */
export function assembleToDom(schema, containerElement = null) {
  ...
  if (normalized.navigation) {
    ...
    if (navConfig.logoUrl) {
      const logo = createElement('img');
      logo.classList.add('od-brand-logo');
      logo.setAttribute('src', sanitizeUrl(navConfig.logoUrl));
      logo.setAttribute('alt', navConfig.brandName || 'Brand Logo');
  ...
  if (normalized.hero) {
    ...
    if (heroConfig.minHeight) {
      heroSection.style.setProperty('min-height', sanitizeCssValue(heroConfig.minHeight, '80vh'));
    }
    ...
    if (heroConfig.baseColor) {
      canvas.setAttribute(
        'data-base-color',
        Array.isArray(heroConfig.baseColor) ? heroConfig.baseColor.join(',') : String(heroConfig.baseColor)
      );
    }
    if (heroConfig.speed) {
      canvas.setAttribute('data-speed', String(heroConfig.speed));
    }
    if (heroConfig.interactiveRadius) {
      canvas.setAttribute('data-radius', String(heroConfig.interactiveRadius));
    }
    ...
    // Live Badge
    if (heroConfig.showLiveBadge) {
      const liveBadge = createElement('span');
      liveBadge.classList.add('od-live-indicator');
      const pulse = createElement('span');
      pulse.classList.add('od-live-pulse');
      liveBadge.appendChild(pulse);
      const liveText = createElement('span');
      liveText.textContent = 'Live Engine';
      liveBadge.appendChild(liveText);
      heroContent.appendChild(liveBadge);
    }
  ...
  if (normalized.bentoGrid) {
    ...
    if (bentoConfig.autoDensity === true || bentoConfig.autoDensity === 'dense') {
      grid.classList.add('od-grid-dense');
      grid.style.setProperty('grid-auto-flow', 'dense');
    }
    const cols = Math.max(1, Math.min(24, Number(bentoConfig.columns) || 12));
    grid.style.setProperty('--bento-columns', String(cols));

    if (bentoConfig.rows) {
      const rows = Math.max(1, Number(bentoConfig.rows));
      grid.style.setProperty('--bento-rows', String(rows));
    ...
    if (Array.isArray(bentoConfig.cellMatrix)) {
      bentoConfig.cellMatrix.forEach((cell) => {
        ...
        if (cell.type) {
          card.classList.add(`od-cell-${cell.type}`);
          card.setAttribute('data-cell-type', cell.type);
        }

        if (cell.className && typeof cell.className === 'string') {
          cell.className.split(/\s+/).forEach((c) => {
            if (/^[a-zA-Z0-9\-_]+$/.test(c)) card.classList.add(c);
          });
        }
        ...
        if (cell.colStart) card.style.setProperty('--col-start', String(Number(cell.colStart)));
        if (cell.rowStart) card.style.setProperty('--row-start', String(Number(cell.rowStart)));
        ...
        if (cell.icon) {
          ...
          iconEl.classList.add('od-cell-icon');
  ...
  if (normalized.scrollytelling) {
    ...
    if (Array.isArray(scrollyConfig.scenes)) {
      ...
      scrollyConfig.scenes.forEach((scene, index) => {
        ...
        if (scene.visualAsset) {
          const visualEl = createElement('div');
          visualEl.classList.add('od-scene-visual');
          if (typeof scene.visualAsset === 'string') {
            const pre = createElement('pre');
            const code = createElement('code');
            code.textContent = scene.visualAsset;
            pre.appendChild(code);
            visualEl.appendChild(pre);
          } else if (typeof scene.visualAsset === 'object') {
            const va = scene.visualAsset;
            if (va.type === 'image' && va.url) {
              const img = createElement('img');
              img.classList.add('od-scene-visual-img');
              img.setAttribute('src', sanitizeUrl(va.url));
              if (va.caption) img.setAttribute('alt', va.caption);
              visualEl.appendChild(img);
            } else if (va.content) {
              const pre = createElement('pre');
              const code = createElement('code');
              code.textContent = va.content;
              pre.appendChild(code);
              visualEl.appendChild(pre);
            }
            if (va.caption) {
              const caption = createElement('figcaption');
              caption.classList.add('od-scene-visual-caption');
              caption.textContent = va.caption;
              visualEl.appendChild(caption);
            }
          }
          sceneCard.appendChild(visualEl);
        }
    ...
    if (scrollyConfig.showProgressIndicator && Array.isArray(scrollyConfig.scenes)) {
      const progressTrack = createElement('div');
      progressTrack.classList.add('od-scrolly-progress');
      scrollyConfig.scenes.forEach((_, idx) => {
        const dot = createElement('span');
        dot.classList.add('od-scrolly-dot');
        if (idx === 0) dot.classList.add('is-active');
        progressTrack.appendChild(dot);
      });
      pinnedViewport.appendChild(progressTrack);
    }
  ...
  if (normalized.soundboard && normalized.soundboard.enabled !== false) {
    ...
    if (sbConfig.volume !== undefined) sbSection.setAttribute('data-volume', String(sbConfig.volume));
    if (sbConfig.muted !== undefined) sbSection.setAttribute('data-muted', String(sbConfig.muted));
    if (sbConfig.spatialPanning !== undefined) {
      sbSection.setAttribute('data-spatial', String(sbConfig.spatialPanning));
    }
  ...
  // 5b. Custom Sections
  // ---------------------------------------------------------------------------
  if (Array.isArray(normalized.customSections) && normalized.customSections.length > 0) {
    normalized.customSections.forEach((sec) => {
      const sectionEl = createElement('section');
      sectionEl.classList.add('od-custom-section');
      if (sec.id) sectionEl.setAttribute('id', sec.id);

      const secInner = createElement('div');
      secInner.classList.add('od-section-inner');

      if (sec.title) {
        const h2 = createElement('h2');
        h2.textContent = sec.title;
        secInner.appendChild(h2);
      }
      if (sec.content) {
        const contentDiv = createElement('div');
        contentDiv.classList.add('od-custom-content');
        contentDiv.textContent = sec.content;
        secInner.appendChild(contentDiv);
      }
      sectionEl.appendChild(secInner);
  ...
  if (normalized.footer) {
    ...
    if (Array.isArray(footerConfig.links) && footerConfig.links.length > 0) {
      ...
      linksContainer.classList.add('od-footer-links');
      footerConfig.links.forEach((l) => {
        const a = createElement('a');
        a.textContent = l.label || 'Link';
        a.setAttribute('href', sanitizeUrl(l.href));
 ...
 * @returns {string}
 */
export function buildStandaloneCss(theme) {
  ...
  if (theme.customTokens && typeof theme.customTokens === 'object') {
    for (const [tokenName, tokenVal] of Object.entries(theme.customTokens)) {
      const cleanVar = tokenName.startsWith('--') ? tokenName : `--${tokenName}`;
      if (/^[a-zA-Z0-9\-_]+$/.test(cleanVar.slice(2))) {
        const sanitizedVal = sanitizeCssValue(tokenVal, '');
        if (sanitizedVal) {
          customTokensCss += `      ${cleanVar}: ${sanitizedVal};\n`;
        }
      }
    }
  }
  ...
  return `
    :root {
      ...
      --od-shadow-md: ${sMd};
      --od-shadow-lg: ${sLg};
${customTokensCss}      --font-sans: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
    ...
    *, *::before, *::after {
    ...
    .od-live-pulse {
      ...
      animation: od-pulse 1.8s infinite cubic-bezier(0.66, 0, 0, 1);
    }
 ...
 * @returns {string}
 */
export function buildStandaloneScript(schema) {
  return `
    (function() {
      'use strict';
      ...
      var sbEl = document.querySelector('.od-soundboard-section');
      var masterVol = sbEl ? parseFloat(sbEl.getAttribute('data-volume') || '0.15') : 0.15;
      var isMuted = sbEl ? sbEl.getAttribute('data-muted') === 'true' : false;
      var isSpatial = sbEl ? sbEl.getAttribute('data-spatial') === 'true' : true;

      function playHapticSound(type, clientX) {
        if (isMuted) return;
        var ctx = getAudioContext();
        if (!ctx) return;
        var now = ctx.currentTime;
        var gainMultiplier = masterVol / 0.15;

        var panner = null;
        if (isSpatial && ctx.createStereoPanner && typeof clientX === 'number') {
          try {
            panner = ctx.createStereoPanner();
            var pan = Math.max(-1, Math.min(1, (clientX / window.innerWidth) * 2 - 1));
        ...
        if (type === 'click' || type === 'true') {
          ...
          gain.gain.setValueAtTime(0.12 * gainMultiplier, now);
          ...
          gain.gain.setValueAtTime(0.15 * gainMultiplier, now);
          ...
          [523.25, 659.25, 783.99].forEach(function(freq, i) {
            ...
            gain.gain.setValueAtTime(0.08 * gainMultiplier, now + i * 0.03);
          ...
          gain.gain.setValueAtTime(0.06 * gainMultiplier, now);
      ...
      document.addEventListener('click', function(e) {
        ...
        if (target) {
          ...
          playHapticSound(soundType, e.clientX);
        }
      });
      ...
      if (canvas && window.requestAnimationFrame) {
        ...
        var baseColor = canvas.getAttribute('data-base-color') || '37, 99, 235';
        var speedMult = parseFloat(canvas.getAttribute('data-speed') || '1.0');
        var interactRadius = parseFloat(canvas.getAttribute('data-radius') || '120');
        ...
        function renderFrame() {
          ...
          ctx2d.fillStyle = 'rgba(' + baseColor + ', 0.45)';
          ctx2d.strokeStyle = 'rgba(' + baseColor + ', 0.12)';
          ctx2d.lineWidth = 1;
      ...
      var dots = Array.from(document.querySelectorAll('.od-scrolly-dot'));
      if (sceneCards.length > 0 && window.IntersectionObserver) {
        var observer = new IntersectionObserver(function(entries) {
          entries.forEach(function(entry) {
            if (entry.isIntersecting) {
              var idx = sceneCards.indexOf(entry.target);
 ...
 * @returns {string}
 */
export function exportToStandaloneHtml(schema) {
  const normalized = normalizeSchema(schema);
  ...
  const authorHtml = meta.author ? `<meta name="author" content="${escapeHtml(meta.author)}">` : '';
  const keywordsHtml = Array.isArray(meta.keywords) && meta.keywords.length > 0
    ? `<meta name="keywords" content="${escapeHtml(meta.keywords.join(', '))}">`
    : '';
  const ogImageHtml = meta.ogImage
    ? `<meta property="og:image" content="${escapeHtml(sanitizeUrl(meta.ogImage))}">`
    : '';
  ...
  if (nav) {
    ...
    const logoHtml = nav.logoUrl
      ? `<img class="od-brand-logo" src="${escapeHtml(sanitizeUrl(nav.logoUrl))}" alt="${brandName}" />`
      : '';
  ...
  if (hero) {
    ...
    const subheadline = hero.subheadline
      ...
      : '';
    const liveIndicatorHtml = hero.showLiveBadge
      ? `<span class="od-live-indicator"><span class="od-live-pulse"></span>Live Engine</span>`
      : '';
    ...
    const heroMinHeightStyle = hero.minHeight
      ? ` style="min-height:${sanitizeCssValue(hero.minHeight, '80vh')};"`
      : '';
    const heroBaseColor = Array.isArray(hero.baseColor)
      ? hero.baseColor.join(',')
      : hero.baseColor || '37,99,235';

    heroHtml = `
      <section class="od-hero-section" id="hero"${heroMinHeightStyle}>
        ...
        )}" data-dpr="${escapeHtml(String(hero.dprCap || 2.0))}" data-base-color="${escapeHtml(
          String(heroBaseColor)
        )}" data-speed="${escapeHtml(String(hero.speed || 1.0))}" data-radius="${escapeHtml(
          String(hero.interactiveRadius || 120)
        )}"></canvas>
  ...
  if (bento) {
    ...
    const isDense = bento.autoDensity === true || bento.autoDensity === 'dense';
    const rows = bento.rows ? Math.max(1, Number(bento.rows)) : null;

    let gridStyle = `--bento-columns:${cols};--bento-gap:${gap};`;
    if (isDense) gridStyle += 'grid-auto-flow:dense;';
    if (rows) gridStyle += `--bento-rows:${rows};grid-template-rows:repeat(${rows}, minmax(0, 1fr));`;
    ...
    const cellsHtml = Array.isArray(bento.cellMatrix)
      ? bento.cellMatrix
          .map((cell) => {
            ...
            const typeAttr = cell.type ? ` data-cell-type="${escapeHtml(cell.type)}"` : '';
            const typeCls = cell.type ? ` od-cell-${escapeHtml(cell.type)}` : '';
            const customCls = cell.className && typeof cell.className === 'string'
              ? ' ' + cell.className.split(/\s+/).filter(c => /^[a-zA-Z0-9\-_]+$/.test(c)).join(' ')
              : '';

            let styleStr = `--col-span:${cSpan};--row-span:${rSpan};`;
            if (cell.colStart) styleStr += `--col-start:${Number(cell.colStart)};grid-column-start:var(--col-start);`;
            if (cell.rowStart) styleStr += `--row-start:${Number(cell.rowStart)};grid-row-start:var(--row-start);`;

            const iconHtml = cell.icon
              ? `<span class="od-cell-icon">${escapeHtml(cell.icon)}</span>`
              : '';
            ...
            return `
              <article class="od-bento-card${typeCls}${customCls}"${idAttr} style="${styleStr}"${spotlightAttr}${tiltAttr}${hapticAttr}${typeAttr}>
    ...
    bentoHtml = `
      <section class="od-bento-section" id="telemetry">
        ${headerHtml}
        <div class="od-bento-grid${isDense ? ' od-grid-dense' : ''}" style="${gridStyle}">
          ${cellsHtml}
        </div>
      </section>
    `;
  ...
  if (scrolly) {
    ...
    const scenesHtml = Array.isArray(scrolly.scenes)
      ? scrolly.scenes
          .map((scene, idx) => {
            ...
            if (scene.visualAsset) {
              if (typeof scene.visualAsset === 'string') {
                visualHtml = `<div class="od-scene-visual"><pre><code>${escapeHtml(scene.visualAsset)}</code></pre></div>`;
              } else if (typeof scene.visualAsset === 'object') {
                const va = scene.visualAsset;
                let imgPart = '';
                if (va.type === 'image' && va.url) {
                  imgPart = `<img class="od-scene-visual-img" src="${escapeHtml(sanitizeUrl(va.url))}" alt="${escapeHtml(va.caption || '')}" />`;
                }
                let contentPart = '';
                if (va.content) {
                  contentPart = `<pre><code>${escapeHtml(va.content)}</code></pre>`;
                }
                let captionPart = '';
                if (va.caption) {
                  captionPart = `<figcaption class="od-scene-visual-caption">${escapeHtml(va.caption)}</figcaption>`;
                }
                visualHtml = `<div class="od-scene-visual">${imgPart}${contentPart}${captionPart}</div>`;
              }
            }
    ...
    let progressHtml = '';
    if (scrolly.showProgressIndicator && Array.isArray(scrolly.scenes)) {
      const dots = scrolly.scenes
        .map((_, i) => `<span class="od-scrolly-dot${i === 0 ? ' is-active' : ''}"></span>`)
        .join('');
      progressHtml = `<div class="od-scrolly-progress">${dots}</div>`;
    }
  ...
  if (soundboard && soundboard.enabled !== false) {
    const volAttr = soundboard.volume !== undefined ? ` data-volume="${soundboard.volume}"` : '';
    const mutedAttr = soundboard.muted !== undefined ? ` data-muted="${soundboard.muted}"` : '';
    const spatialAttr = soundboard.spatialPanning !== undefined ? ` data-spatial="${soundboard.spatialPanning}"` : '';

    soundboardHtml = `
      <section class="od-soundboard-section" id="soundboard"${volAttr}${mutedAttr}${spatialAttr}>
  ...
  if (Array.isArray(normalized.customSections) && normalized.customSections.length > 0) {
    customSectionsHtml = normalized.customSections
      .map((sec) => {
        const idAttr = sec.id ? ` id="${escapeHtml(sec.id)}"` : '';
        const titlePart = sec.title ? `<h2>${escapeHtml(sec.title)}</h2>` : '';
        const contentPart = sec.content ? `<div class="od-custom-content">${escapeHtml(sec.content)}</div>` : '';
        return `
          <section class="od-custom-section"${idAttr}>
            <div class="od-section-inner">
              ${titlePart}
              ${contentPart}
            </div>
          </section>
  ...
  if (footer) {
    ...
    const badgesHtml = Array.isArray(footer.badges)
      ? `<div class="od-footer-badges">${footer.badges
          ...
          .join('')}</div>`
      : '';
    const linksHtml = Array.isArray(footer.links) && footer.links.length > 0
      ? `<div class="od-footer-links">${footer.links
          .map((l) => `<a href="${escapeHtml(sanitizeUrl(l.href))}">${escapeHtml(l.label)}</a>`)
          .join('')}</div>`
      : '';
...
export default SafeDOMAssembler;
